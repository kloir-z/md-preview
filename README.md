...
Monokaiテーマ + highlight.jsコードハイライト。ファイル変更時に自動リロード。

## 必要なもの

- Python 3.10+
- `markdown` パッケージ
- [NSSM](https://nssm.cc/)（Windowsサービス化する場合）
...
> **Note:** 依存が `markdown` 1パッケージのみのため、venvは使わずグローバルインストールで運用している。NSSMサービスからもシステムのPythonを直接参照するため、venvを挟むと設定が複雑になる。
